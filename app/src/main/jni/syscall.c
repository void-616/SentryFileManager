...
static jclass findClassViaLoader(JNIEnv *env, const char *name);

static jclass findClass(JNIEnv *env, const char *name) {
    jclass localClass = (*env)->FindClass(env, name);
    if (!localClass) { (*env)->ExceptionClear(env); localClass = findClassViaLoader(env, name); }
    if (!localClass) {
...
static jclass getSyscallExceptionClass(JNIEnv *env) {
    ...
    if (!syscallExceptionClass) {
        syscallExceptionClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/SyscallException");
    }
...
static jclass getByteStringClass(JNIEnv *env) {
    ...
    if (!byteStringClass) {
        byteStringClass = findClass(env, "com/sentry/filemanager/provider/common/ByteString");
    }
...
static jclass getInt32RefClass(JNIEnv *env) {
    ...
    if (!int32RefClass) {
        int32RefClass = findClass(env, "com/sentry/filemanager/provider/linux/syscall/Int32Ref");
    }
...
static jclass getStructDirentClass(JNIEnv *env) {
    ...
    if (!structStatClass) {
        structStatClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructDirent");
    }
...
static jclass getStructGroupClass(JNIEnv *env) {
    ...
    if (!structGroupClass) {
        structGroupClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructGroup");
    }
...
static jclass getStructInotifyEventClass(JNIEnv *env) {
    ...
    if (!structInotifyEventClass) {
        structInotifyEventClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructInotifyEvent");
    }
...
static jclass getStructMntentClass(JNIEnv *env) {
    ...
    if (!structMntentClass) {
        structMntentClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructMntent");
    }
...
static jfieldID getStructMntentMntOptsField(JNIEnv *env) {
    ...
    if (!structMntentMntOptsField) {
        structMntentMntOptsField = findField(env, getStructMntentClass(env), "mnt_opts",
                "Lcom/sentry/filemanager/provider/common/ByteString;");
    }
...
static jclass getStructPasswdClass(JNIEnv *env) {
    ...
    if (!structPasswdClass) {
        structPasswdClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructPasswd");
    }
...
static jclass getStructStatClass(JNIEnv *env) {
    ...
    if (!structStatClass) {
        structStatClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructStat");
    }
    return structStatClass;
}
...
static jclass getStructTimespecClass(JNIEnv *env) {
    ...
    if (!structTimespecClass) {
        structTimespecClass = findClass(env,
                "com/sentry/filemanager/provider/linux/syscall/StructTimespec");
    }
...
static void throwSyscallException(JNIEnv* env, const char* functionName) {
    int error = errno;
    // Try to throw SyscallException; fall back to IOException if class lookup fails
    jclass exceptionClass = getSyscallExceptionClass(env);
    if (!exceptionClass || (*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        // Fallback: throw a plain IOException
        jclass ioExceptionClass = (*env)->FindClass(env, "java/io/IOException");
        if (ioExceptionClass) {
            char msg[256];
            snprintf(msg, sizeof(msg), "%s failed: %s (errno=%d)", functionName, strerror(error), error);
...
static char *mallocStringFromByteString(JNIEnv *env, jobject javaByteString) {
    ...
    if (javaByteString == NULL) {
        errno = EINVAL;
        return NULL;
    }

    jfieldID field = getByteStringBytesField(env);
    if (field == NULL) {
        errno = EINVAL;
        return NULL;
    }
    ...
    jbyteArray javaBytes =
            (*env)->GetObjectField(env, javaByteString, field);
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_access(
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_chmod(
        JNIEnv *env, jclass clazz, jobject javaPath, jint javaMode) {
    char *path = mallocStringFromByteString(env, javaPath);
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_chown(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_closedir(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_endmntent(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_endgrent(JNIEnv *env, jclass clazz) {
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_endpwent(JNIEnv *env, jclass clazz) {
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_errno(
        JNIEnv *env, jclass clazz) {
    return errno;
}

JNIEXPORT jint JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_fcntl_1int(
...
JNIEXPORT jint JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_fcntl_1void(
...
static jobject newStructGroup(JNIEnv *env, const struct group *group) {
    ...
    if (!constructor) {
        constructor = findMethod(env, getStructGroupClass(env), "<init>",
                                 "(Lcom/sentry/filemanager/provider/common/ByteString;"
                                 "Lcom/sentry/filemanager/provider/common/ByteString;I"
                                 "[Lcom/sentry/filemanager/provider/common/ByteString;)V");
    }
    jobject gr_name;
    if (group->gr_name) {
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getgrent(JNIEnv *env, jclass clazz) {
    while (true) {
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getgrgid(
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getgrnam(
...
static jobject newStructMntent(JNIEnv *env, const struct mntent *mntent) {
    ...
    if (!constructor) {
        constructor = findMethod(env, getStructMntentClass(env), "<init>",
                                 ...
                                 "Lcom/sentry/filemanager/provider/common/ByteString;"
                                 "Lcom/sentry/filemanager/provider/common/ByteString;"
                                 "Lcom/sentry/filemanager/provider/common/ByteString;II)V");
    }
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getmntent(
        JNIEnv *env, jclass clazz, jlong javaFile) {
    FILE *file = (FILE *) javaFile;
#if __ANDROID_API__ >= __ANDROID_API_L_MR1__
...
static jobject newStructPasswd(JNIEnv *env, const struct passwd *passwd) {
    ...
    if (!constructor) {
        constructor = findMethod(env, getStructPasswdClass(env), "<init>",
                "(Lcom/sentry/filemanager/provider/common/ByteString;II"
                "Lcom/sentry/filemanager/provider/common/ByteString;"
                "Lcom/sentry/filemanager/provider/common/ByteString;"
                "Lcom/sentry/filemanager/provider/common/ByteString;)V");
    }
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getpwent(JNIEnv *env, jclass clazz) {
    while (true) {
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getpwnam(
        JNIEnv *env, jclass clazz, jobject javaName) {
    char *name = mallocStringFromByteString(env, javaName);
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_getpwuid(
...
JNIEXPORT jboolean JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_hasmntopt(
...
JNIEXPORT jint JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_inotify_1add_1watch(
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_inotify_1init1(
...
static jobject newStructInotifyEvent(JNIEnv *env, const struct inotify_event *event) {
    ...
    if (!constructor) {
        constructor = findMethod(env, getStructInotifyEventClass(env), "<init>",
                                 "(IIILcom/sentry/filemanager/provider/common/ByteString;)V");
    }
    jint wd = event->wd;
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_inotify_1get_1events(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_inotify_1rm_1watch(
...
JNIEXPORT jint JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_ioctl_1int(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_lchown(
        JNIEnv *env, jclass clazz, jobject javaPath, jint javaUid, jint javaGid) {
    char *path = mallocStringFromByteString(env, javaPath);
    uid_t uid = (uid_t) javaUid;
    gid_t gid = (gid_t) javaGid;
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_lgetxattr(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_link(
...
JNIEXPORT jobjectArray JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_llistxattr(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_lsetxattr(
...
static jobject newStructStat(JNIEnv *env, const struct stat64 *stat) {
    ...
    if (!constructor) {
        constructor = findMethod(env, getStructStatClass(env), "<init>", "(JJIJIIJJJJ"
                "Lcom/sentry/filemanager/provider/linux/syscall/StructTimespec;"
                "Lcom/sentry/filemanager/provider/linux/syscall/StructTimespec;"
                "Lcom/sentry/filemanager/provider/linux/syscall/StructTimespec;)V");
    }
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_lstat(
        JNIEnv *env, jclass clazz, jobject javaPath) {
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_lutimens(
        JNIEnv *env, jclass clazz, jobject javaPath, jobjectArray javaTimes) {
    doUtimens(env, javaPath, javaTimes, true);
}

JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_mkdir(
        JNIEnv *env, jclass clazz, jobject javaPath, jint javaMode) {
    char *path = mallocStringFromByteString(env, javaPath);
    mode_t mode = (mode_t) javaMode;
...
JNIEXPORT jint JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_mount(
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_open(
...
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_opendir(
        JNIEnv *env, jclass clazz, jobject javaPath) {
...
static jobject newStructDirent(JNIEnv *env, const struct dirent64 *dirent) {
    ...
    if (!constructor) {
        constructor = findMethod(env, getStructDirentClass(env), "<init>",
                                 "(JJIILcom/sentry/filemanager/provider/common/ByteString;)V");
    }
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_readdir(
        JNIEnv *env, jclass clazz, jlong javaDir) {
    DIR *dir = (DIR *) javaDir;
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_readlink(
        JNIEnv *env, jclass clazz, jobject javaPath) {
    char *path = mallocStringFromByteString(env, javaPath);
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_realpath(
        JNIEnv *env, jclass clazz, jobject javaPath) {
    char *path = mallocStringFromByteString(env, javaPath);
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_remove(
        JNIEnv *env, jclass clazz, jobject javaPath) {
    char *path = mallocStringFromByteString(env, javaPath);
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_rename(
        JNIEnv *env, jclass clazz, jobject javaOldPath, jobject javaNewPath) {
    char *oldPath = mallocStringFromByteString(env, javaOldPath);
    char *newPath = mallocStringFromByteString(env, javaNewPath);
...
JNIEXPORT jlong JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_sendfile(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_setgrent(JNIEnv *env, jclass clazz) {
...
JNIEXPORT jlong JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_setmntent(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_setpwent(JNIEnv *env, jclass clazz) {
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_stat(
        JNIEnv *env, jclass clazz, jobject javaPath) {
...
JNIEXPORT jobject JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_statvfs(
        JNIEnv *env, jclass clazz, jobject javaPath) {
    char *path = mallocStringFromByteString(env, javaPath);
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_symlink(
...
JNIEXPORT void JNICALL
Java_com_sentry_filemanager_provider_linux_syscall_Syscall_utimens(
        JNIEnv *env, jclass clazz, jobject javaPath, jobjectArray javaTimes) {
...
static jmethodID gLoadClassMethod = NULL;

static jclass findClassViaLoader(JNIEnv *env, const char *name) {
    if (!gClassLoader || !gLoadClassMethod) return NULL;
    // Convert dots to slashes back to dots for loadClass (loadClass uses dots)
    char dotName[512];
    strncpy(dotName, name, sizeof(dotName)-1);
    for (int i = 0; dotName[i]; i++) {
        if (dotName[i] == '/') dotName[i] = '.';
    }
    jstring className = (*env)->NewStringUTF(env, dotName);
    if (!className) return NULL;
    jclass result = (jclass)(*env)->CallObjectMethod(env, gClassLoader, gLoadClassMethod, className);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    // Cache app classloader — must be done on main thread during library load
    jclass someClass = (*env)->FindClass(env, "com/sentry/filemanager/provider/linux/syscall/Syscall");
    if (someClass) {
        jclass classClass = (*env)->FindClass(env, "java/lang/Class");
        jmethodID getClassLoader = (*env)->GetMethodID(env, classClass, "getClassLoader", "()Ljava/lang/ClassLoader;");
        jobject classLoader = (*env)->CallObjectMethod(env, someClass, getClassLoader);
        gClassLoader = (*env)->NewGlobalRef(env, classLoader);
        jclass classLoaderClass = (*env)->FindClass(env, "java/lang/ClassLoader");
        gLoadClassMethod = (*env)->GetMethodID(env, classLoaderClass, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");
