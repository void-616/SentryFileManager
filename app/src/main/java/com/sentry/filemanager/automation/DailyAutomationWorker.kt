    ...
    override fun doWork(): Result {
        return try {
            val rules = AutomationRuleStore.getRules(applicationContext)
                .filter { it.enabled && it.trigger == RuleTrigger.SCHEDULED_DAILY }
            ...
            rules.forEach { rule ->
                rule.actions.forEach { action ->
                    ...
                    AutomationRuleStore.addLog(
                        ...
                        RuleRunLog(
                            ruleId = rule.id,
                            triggerType = RuleTrigger.SCHEDULED_DAILY,
                ...
                if (AutomationEngine.isTaskerBridgeEnabled(applicationContext)) {
                    applicationContext.sendBroadcast(
                        android.content.Intent(AutomationEngine.ACTION_RULE_TRIGGERED).apply {
                            ...
                            putExtra(AutomationEngine.EXTRA_TRIGGER, RuleTrigger.SCHEDULED_DAILY.name)
                        }
                    )
                }
            }
